...
function FindPlayers() {
  ...
  const [selectedTeams, setSelectedTeams] = useState({}); // ✅ track team selected per player
  ...
  const handleInvite = async (playerSteamId) => {
    ...
    try {
      ...
      console.log(`✅ Invited ${playerSteamId} to ${teamName}`);
    } catch (error) {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {players.map((player) => (
            ...
            >
              ...
              {/* Invite Button */}
              {inviteStatus[player.steamId] === "invited" ? (
                <button
                  disabled
                  className="bg-green-500/40 text-black font-bold py-2 px-4 rounded-full opacity-70 cursor-not-allowed"
                >
                  Invited ✅
                </button>
              ) : (
